  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3">
        {notesList.map((note) => (<Card key={note.id} className="flex items-center justify-between p-4">
        ...
        {notesList.length === 0 && <div className="text-text-muted text-sm p-4">{t(lang, "noData")}</div>}
      </div>
    </div>
  );
}
